...
local Dictionary_file = {
	["en-US"] = { "/Users/n3m0/.config/nvim/" .. "spell/dictionary.txt" } -- there is another way to find ~/.config/nvim ?
}
local DisabledRules_file = {
	["en-US"] = { "/Users/n3m0/.config/nvim/" .. "spell/disable.txt" } -- there is another way to find ~/.config/nvim ?
}
local FalsePositives_file = {
	["en-US"] = { "/Users/n3m0/.config/nvim/" .. "spell/false.txt" } -- there is another way to find ~/.config/nvim ?
}
...
local function updateConfig(lang, configtype)
	local buf_clients = vim.lsp.buf_get_clients()
	local client = nil
	for _, lspin in ipairs(buf_clients) do
		if lspin.name == "ltex" then
			client = lspin
		end
	end
...
if not lsp.ltex then
	configs.ltex = {
		default_config = {
			cmd = { "ltex-ls" },
			filetypes = { 'tex', 'bib', 'md', 'typ' },
			root_dir = function(filename)
				return util.path.dirname(filename)
			end,
			settings = {
				ltex = {
					enabled = { "latex", "tex", "bib", "md", "typ", "typst" },
					checkFrequency = "save",
					language = "en-US",
...
lsp.ltex.disabledrules_file = DisabledRules_file
lsp.ltex.falsepostivies_file = FalsePositives_file
